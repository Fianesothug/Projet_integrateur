...
<body>
    <div class="container">
        <h1 class="hh"><a href="#" onclick="history.back()">Retour</a></h1>
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['inscription'])) {
            ...
            $code = $password; // On stocke le mot de passe en clair (sans hachage)
            $matricule = strtoupper('M148'. $random_suffix); // Matricule basé sur les initiales et le suffixe
            ...
            try {
                ...
                $check_sql = "SELECT * FROM personnes WHERE identifiant = :identifiant OR email = :email";
                ...
                // Requête d'insertion (corrigé pour inclure le champ code)
                $sql = "INSERT INTO personnes (nom, prenom, sexe, pays, numero, email, adresse, identifiant, matricule, code) 
                        VALUES (:nom, :prenom, :sexe, :pays, :numero, :email, :adresse, :identifiant, :matricule, :code)";
                ...
                if (!$insert_success) {
                    echo "<h2 class='error'>Échec de l'inscription dans la base de données.</h2>";
                    exit;
                }

                // Configuration de l'email (uniquement si l'insertion a réussi)
...
<body>
    <div class='container'>
        ...
        <div class='content'>
            ...
            <p><strong>Numéro matricule :</strong> $matricule</p>
...
";
                ...
                // Si l'email n'a pas été envoyé, on supprime l'entrée de la base de données
                if (!$email_sent) {
                    // Suppression de l'entrée précédemment insérée
                    $delete_sql = "DELETE FROM personnes WHERE email = :email AND identifiant = :identifiant";
                    $delete_stmt = $pdo->prepare($delete_sql);
                    $delete_stmt->bindParam(':email', $email);
                    $delete_stmt->bindParam(':identifiant', $identifiant);
                    $delete_stmt->execute();
                    
                    echo "<h2 class='error'>Le système d'envoi d'emails est temporairement indisponible. Veuillez réessayer avec une connexion à internet ou contacter l'administrateur.</h2>";
                    exit;
                }

                // Si tout s'est bien passé (insertion et envoi d'email)
                ...
                echo "<h2>Votre numero matricule est : 👉 $matricule 👈</h2>";
                echo "<h2>Votre mot de passe(ajout de caractères spéciaux, pour plus de sécurité) est : 👉 voir l'email 👈</h2>";
